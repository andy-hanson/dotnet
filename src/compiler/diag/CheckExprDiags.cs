...
static class ShowUtils {
	internal static StringMaker addMember(this StringMaker s, Member m) =>
		s.add(m.klass.name.str).add('.').add(m.name.str);
...
namespace Diag.CheckExprDiags {
	...
	internal sealed class CantSetNonSlot : Diag<CantSetNonSlot> {
		...
		internal override void show(StringMaker s) =>
			s.addMember(member).add(" is not a slot; can't be set.");
	...
	internal sealed class SlotNotMutable : Diag<SlotNotMutable> {
		...
		internal override void show(StringMaker s) =>
			s.add("Slot ").addMember(slot).add(" is not mutable.");
	...
	internal sealed class MissingEffectToSetSlot : Diag<MissingEffectToSetSlot> {
		...
		internal override void show(StringMaker s) =>
			s.add("Slot ").addMember(slot).add(" can't be set through a reference that doesn't have the 'set' effect.");
	...
	internal sealed class MissingEffectToGetSlot : Diag<MissingEffectToGetSlot> {
		...
		internal override void show(StringMaker s) =>
			s.add("Slot ").addMember(slot).add(" is mutable, and can't be read through a pure reference.");
	...
	internal sealed class ArgumentCountMismatch : Diag<ArgumentCountMismatch> {
		...
		internal override void show(StringMaker s) =>
			s.add("Method ").addMember(called).add(" takes ").add(called.parameters.length).add(" arguments; got ").add(argumentsCount);
	...
	internal sealed class CantAccessSlotFromStaticMethod : Diag<CantAccessSlotFromStaticMethod> {
		...
		internal override void show(StringMaker s) =>
			s.add("Slot ").addMember(slot).add("can't be accessed from a static method.");
	...
	internal sealed class CantCallInstanceMethodFromStaticMethod : Diag<CantCallInstanceMethodFromStaticMethod> {
		...
		internal override void show(StringMaker s) =>
			s.addMember(method).add(" is an instance method; can't be called from a static method.");
	...
	internal sealed class CantAccessStaticMethodThroughInstance : Diag<CantAccessStaticMethodThroughInstance> {
		...
		internal override void show(StringMaker s) =>
			s.addMember(method).add(" is a static method; it should not be called like an instance method.");

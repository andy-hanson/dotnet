...
namespace Model {
	...
	internal static class BuiltinMethod {
		internal static MethodDeclaration of(BuiltinClass klass, MethodInfo method) {
			bool isStatic;
			...
			var returnTy = Ty.of(returnEffect, BuiltinsLoader.instClsFromDotNetType(method.ReturnType));
			...
			if (method.ContainsGenericParameters)
				throw TODO();
			assert(method.GetGenericArguments().Length == 0);
			var typeParameters = Arr.empty<TypeParameter>();

			return method.IsAbstract
				? new BuiltinAbstractMethod(klass, method, typeParameters, returnTy, name, selfEffect, @params).upcast<MethodDeclaration>()
				: new BuiltinMethodWithBody(isStatic, klass, method, typeParameters, returnTy, name, selfEffect, @params);
		}
		...
		static Parameter getParam(ParameterInfo p, uint index, bool allPure) {
			...
			var cls = BuiltinsLoader.instClsFromDotNetType(p.ParameterType);
	...
	internal sealed class BuiltinMethodWithBody : MethodWithBodyLike, ToData<BuiltinMethodWithBody>, IEquatable<BuiltinMethodWithBody> {
		...
		internal BuiltinMethodWithBody(bool isStatic, BuiltinClass klass, MethodInfo methodInfo, Arr<TypeParameter> typeParameters, Ty returnTy, Sym name, Effect selfEffect, Arr<Parameter> @params)
			: base(klass, Loc.zero, typeParameters, returnTy, name, selfEffect, @params) {
		...
		public override bool deepEqual(MethodDeclaration m) => m is BuiltinMethodWithBody b && deepEqual(b);
	...
	internal sealed class BuiltinAbstractMethod : AbstractMethodLike, ToData<BuiltinAbstractMethod> {
		...
		internal BuiltinAbstractMethod(BuiltinClass klass, MethodInfo methodInfo, Arr<TypeParameter> typeParameters, Ty returnTy, Sym name, Effect selfEffect, Arr<Parameter> @params)
			: base(klass, Loc.zero, typeParameters, returnTy, name, selfEffect, @params) {
		...
		internal override MemberId getMemberId() => throw new NotSupportedException();
		internal override string showKind(bool upper) => throw new NotSupportedException();
		public override bool deepEqual(MethodDeclaration m) => m is BuiltinAbstractMethod b && deepEqual(b);

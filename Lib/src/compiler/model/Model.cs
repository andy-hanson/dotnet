...
namespace Model {
	...
	sealed class Module : ToData<Module>, Identifiable<Path> {
		/**
		For "a.nz" this is "a".
		...
		*/
		...
		public bool Equals(Module m) =>
			...
			imports.eq(m.imports, IdentifiableU.equalsId<Module, Path>) &&
	...
	abstract class Ty : ToData<Ty>, Identifiable<ClassLike.Id> {
		public override bool Equals(object o) => o is Ty t && Equals(t);
		public abstract bool Equals(Ty ty);
		public override abstract int GetHashCode();
		public abstract Dat toDat();
		public static bool operator ==(Ty a, Ty b) => a.Equals(b); //TODO: kill, use Equals
	...
	abstract class ClassLike : Ty, Identifiable<ClassLike.Id> {
		...
		internal struct Id : ToData<Id> {
			...
			public bool Equals(Id i) => id.Equals(i.id);
	...
	sealed class BuiltinClass : ClassLike, ToData<BuiltinClass> {
		...
		public override bool Equals(Ty ty) => throw TODO();//object.ReferenceEquals(this, ty);
		public bool Equals(BuiltinClass b) => throw TODO();//object.ReferenceEquals(this, b);
	...
	sealed class Klass : ClassLike, ToData<Klass> {
		...
		public override bool Equals(Ty ty) => ty is Klass k && Equals(k);
		public bool Equals(Klass k) => throw TODO();//object.ReferenceEquals(this, k);
		public override int GetHashCode() => name.GetHashCode();
		public override Dat toDat() => Dat.of(this, nameof(loc), loc, nameof(head), head, nameof(membersMap), Dat.dict(membersMap));

		internal abstract class Head : ToData<Head> {
			readonly Loc loc;
			Head(Loc loc) { this.loc = loc; }

			public abstract bool Equals(Head head);
			public abstract Dat toDat();

			// Static class: May only contain "fun"
			internal class Static : Head, ToData<Static> {
				internal Static(Loc loc) : base(loc) {}
				public override bool Equals(Head h) => h is Static s && Equals(s);
				public bool Equals(Static s) => loc.Equals(s.loc);
			...
			internal class Abstract : Head, ToData<Abstract> {
				...
				public override bool Equals(Head h) => h is Abstract a && Equals(a);
				public bool Equals(Abstract a) => loc.Equals(a.loc);
				public override Dat toDat() => Dat.of(this, nameof(loc), loc);
			}
			...
			internal class Slots : Head, ToData<Slots> {
				...
				public override bool Equals(Head h) => h is Slots s && Equals(s);
				public bool Equals(Slots s) => slots.eq(s.slots);
				...
				internal sealed class Slot : Member, ToData<Slot>, Identifiable<Slot.Id> {
					internal struct Id : ToData<Id> {
						...
						public bool Equals(Id i) => klass.Equals(i) && name.Equals(i.name);
					...
					public override bool Equals(Member m) => m is Slot s && Equals(s);
					public bool Equals(Slot s) => loc.Equals(s.loc) && name.Equals(s.name) && mutable == s.mutable && ty.Equals(s.ty);
	...
	abstract class Member : ToData<Member> {
		internal readonly Loc loc;
		internal readonly Sym name;
		internal Member(Loc loc, Sym name) { this.loc = loc; this.name = name; }
		public abstract bool Equals(Member m);
		public abstract Dat toDat();
	}
	...
	abstract class Method : Member, ToData<Method>, Identifiable<Method.Id> {
		internal struct Id : ToData<Id> {
			...
			internal readonly Sym name;
			internal Id(ClassLike.Id klass, Sym name) { this.klass = klass; this.name = name; }
			public bool Equals(Id m) => klass.Equals(m.klass) && name.Equals(m.name);
			public Dat toDat() => Dat.of(this, nameof(klass), klass, nameof(name), name);
		}
		...
		public abstract bool Equals(Method m);
		public Id getId() => new Id(klass.getId(), name);
		...
		internal sealed class Parameter : ToData<Parameter> {
			internal readonly Loc loc;
			internal readonly Ty ty;
			internal readonly Sym name;
			...
			public bool Equals(Parameter p) => loc.Equals(p.loc) && ty.Equals(p.ty) && name.Equals(p.name) && index == p.index;
		...
		internal sealed class BuiltinMethod : Method, ToData<BuiltinMethod> {
			...
			public override bool Equals(Member m) => m is BuiltinMethod b && Equals(b);
			public override bool Equals(Method m) => m is BuiltinMethod b && Equals(b);
			public bool Equals(BuiltinMethod m) =>
				name.Equals(m.name) && isStatic == m.isStatic && returnTy.Equals(m.returnTy) && parameters.eq(m.parameters);
		...
		internal sealed class MethodWithBody : Method, ToData<MethodWithBody> {
			...
			public override bool Equals(Member m) => m is MethodWithBody mb && Equals(mb);
			public override bool Equals(Method m) => m is MethodWithBody mb && Equals(mb);
			public bool Equals(MethodWithBody m) => throw TODO(); // TODO: should methods with different (reference identity) parent be not equal?
	...
	abstract class Pattern : ToData<Pattern> {
		internal readonly Loc loc;
		...
		internal sealed class Ignore : Pattern, ToData<Ignore> {
			...
			public override bool Equals(Pattern p) => p is Ignore i && Equals(i);
			public bool Equals(Ignore i) => loc.Equals(i.loc);
			public override Dat toDat() => Dat.of(this, nameof(loc), loc);
		}
		internal sealed class Single : Pattern, ToData<Single>, Identifiable<Sym> {
			...
			public override bool Equals(Pattern p) => p is Single s && Equals(s);
			public bool Equals(Single s) => loc.Equals(s.loc) && ty.Equals(s.ty) && name.Equals(s.name);
		...
		internal sealed class Destruct : Pattern, ToData<Destruct> {
			...
			public override bool Equals(Pattern p) => p is Destruct d && Equals(d);
			public bool Equals(Destruct d) => loc.Equals(d.loc) && destructuredInto.eq(d.destructuredInto);

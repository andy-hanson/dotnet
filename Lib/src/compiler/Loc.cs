...
struct PathLoc : ToData<PathLoc> {
	...
	public bool Equals(PathLoc p) => path.Equals(p.path) && loc.Equals(p.loc);
...
struct Loc : ToData<Loc>, ToCsonSpecial {
	...
	public bool Equals(Loc l) => start == l.start && end == l.end;
...
struct LineAndColumn : ToData<LineAndColumn> {
	...
	public bool Equals(LineAndColumn l) => line == l.line && column == l.column;
...
struct LineAndColumnLoc : ToData<LineAndColumnLoc> {
	...
	public bool Equals(LineAndColumnLoc l) => start.Equals(l.start) && end.Equals(l.end);
